...
**Goal:** Wire all subsystems into a single asyncio entry point with a task supervisor, 250Hz controller loop, MIDI dispatch, state broadcast, and clean shutdown.

**File:** `src/main.py`
**Tests:** No isolated unit tests — integration is the test. Run the full system.
**Dependencies:** All other `src/` modules must be complete first.

---

## Task 1: `LatestValueChannel` — overwrite-on-full queue

**Step 1:** Write test (add to `tests/test_mapping.py` or a new `tests/test_main.py`)
...
**Step 3:** Implement `LatestValueChannel` in `src/main.py`
...
## Task 2: `controller_loop` coroutine

Implement in `src/main.py`. This is the 250Hz heartbeat:
...
## Task 3: `broadcast_loop` coroutine
...
## Task 4: `supervised_task` wrapper
...
## Task 6: Smoke test — run the system

With the DualSense connected and Mixxx installed:
...
# Expected output:
# DualSense DJ ready. Connect Mixxx to 'DualSense DJ' MIDI device.
```

Verify:
- No immediate crash
- LED on controller turns dim blue
- `aconnect -l` shows "DualSense DJ" port
- Moving triggers or sticks produces no errors in console
- Open Mixxx → Preferences → Controllers → "DualSense DJ" appears

Press Ctrl+C and verify clean shutdown message appears.
