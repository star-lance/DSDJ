...
def test_cc_value_clamped_to_zero_for_negative(bridge_and_mock):
    bridge, mock_out = bridge_and_mock
    bridge.send_cc(channel=0, cc=0x07, value=-10)
    args = mock_out.send_message.call_args[0][0]
    assert args[2] == 0
...
def test_send_action_volume_deck_a(bridge_and_mock):
    bridge, mock_out = bridge_and_mock
...
def test_send_action_play_pause_deck_b(bridge_and_mock):
    bridge, mock_out = bridge_and_mock
...
def test_send_action_hot_cue_3_deck_a(bridge_and_mock):
    bridge, mock_out = bridge_and_mock
...
def test_send_action_effect_uses_gyro_binding(bridge_and_mock):
    bridge, mock_out = bridge_and_mock
